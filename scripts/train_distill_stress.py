#!/usr/bin/env python3
"""Train RGB-only student via distillation from multimodal teacher for stress detection."""
...
from src.training.distill_stress_loss import StressDistillationLoss
...
def train_fold(
    ...
    """Train a single fold with distillation.
    ...
    # Create student (RGB-only)
    student = StressDetectionModel(cfg)  # cfg has ablation.enabled_modalities: [image]
    student.to(device)
    
    # Create distillation loss
    criterion = StressDistillationLoss(
        ...
        pos_weight=getattr(cfg.training, 'pos_weight', None),
    )
    
    # Create optimizer
    optimizer = torch.optim.AdamW(
        student.parameters(),  # Only train student
        lr=cfg.training.lr,
    ...
    if resume_from:
        ...
        student.load_state_dict(checkpoint['model_state_dict'])  # Load student state
    ...
    # Training info
    print(f"\n{'='*80}")
    print(f"Training fold {fold_id} with distillation")
    print(f"  Teacher: {teacher_checkpoint_path}")
    print(f"  Student modalities: {cfg.model.ablation.enabled_modalities}")
    ...
    for epoch in range(current_epoch, max_epochs):
        ...
        # Update alpha for distillation loss
        criterion.set_alpha(epoch, max_epochs)
        ...
        for batch in train_loader:
            ...
            with torch.autocast('cuda', dtype=amp_dtype):
                ...
                with torch.no_grad():
                    teacher_preds = teacher(batch)
                
                # Student forward
                student_preds = student(batch)
                
                # Distillation loss
                loss, loss_dict = criterion(student_preds, teacher_preds, batch)
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(student.state_dict(), model_only_path)
    ...
    if best_model_path.exists():
        student.load_state_dict(torch.load(best_model_path, map_location=device, weights_only=True))
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            # Student forward only (teacher not needed for test metrics)
            student_preds = student(batch)
            teacher_preds = teacher(batch)  # Still compute for loss consistency
            loss, loss_dict = criterion(student_preds, teacher_preds, batch)
            
            test_loss += loss.item()
            num_test_batches += 1
            
            # Collect student modality gates
            gates = student_preds['modality_gates'].cpu()  # (B, T, 4)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description='Train RGB-only student via distillation from multimodal teacher'
    )
    parser.add_argument(
        '--config',
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Teacher checkpoint base directory (default from config)',
    )
    parser.add_argument(
        '--feature_dir',
        type=str,
    ...
    for fold_id, (train_indices, val_indices, test_indices) in enumerate(cv.split()):
        ...
        # Resolve teacher checkpoint path for this fold
        if args.teacher_dir:
            teacher_base = Path(args.teacher_dir)
        else:
            teacher_base = Path(cfg.distillation.teacher_checkpoint_dir)
        teacher_checkpoint_path = teacher_base / f"fold_{fold_id}" / "best_model_state.pt"
